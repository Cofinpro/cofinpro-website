...
class Template extends React.Component {
  ...
  constructor(props) {
    ...
    this.perspectives = ["studenten", "technologischer-absolvent", "technologischer-professional", "fachlicher-absolvent", "fachlicher-professional", "andere"];
    ...
    if (props.location.pathname.slice(1).split('/').length > 0 && this.perspectives.indexOf(props.location.pathname.slice(1).split('/')[urlFragmentPers]) > -1) {
      perspectiveFromUrl = props
        .location
        .pathname
        .slice(1)
        .split('/')[urlFragmentPers]
    ...
    this.state = {
      ...
      visible: perspectiveFromUrl.length === 0
        ? false
        : true,
      ...
      .listen((location, action) => {
        ...
        if (location.pathname.slice(1).split('/').length > 0 && this.perspectives.indexOf(location.pathname.slice(1).split('/')[urlFragmentPers]) > -1) {
          StorageHelper.saveInSessionStorage('perspective', location.pathname.slice(1).split('/')[urlFragmentPers]);
        }
        ...
        if(this.state.locationUpdate !== location.pathname || this.state.perspective !== StorageHelper.getFromSessionStorage('perspective')) {
          this.setState({
