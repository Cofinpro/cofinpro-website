...
class CarrerOffersCarousel extends React.Component {
    ...
    render() {
        const { id, itemPerSlideLimiter, itemsData, perspective, location, blacklistedItem } = this.props
        ...
        function CarouselInner(props) {
            ...
            for (var i = 1; i < itemsData.length; i++) {
                ...
                if ((perspective == null || perspective.trim().length < 1) || (itemsData[i - 1].node.perspektiveLink.name === perspective)) {
                    
                    if(blacklistedItem == null || blacklistedItem !== itemsData[i - 1].node.id) {
                        tempBucket.push(itemsData[i - 1]);
                    }
                    
                }

                if ((tempBucket.length > 0 && tempBucket.length % itemPerSlideLimiter === 0) || ((i + 1) === itemsData.length && tempBucket.length > 0)) {
        ...
        return (
            <div
                ...
                <ol className="carousel-indicators">
                    <li data-target={"#" + id + "_carouselCaptions"} data-slide-to="0" className=""></li>
                    <li data-target={"#" + id + "_carouselCaptions"} data-slide-to="1" className=""></li>
                    <li data-target={"#" + id + "_carouselCaptions"} data-slide-to="2" className="active"></li>
                </ol>
